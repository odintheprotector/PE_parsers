...
void printExportDirectory(PIMAGE_NT_HEADERS64 imageNTHeaders, DWORD fileBuffer);
...
void printExportDirectory(PIMAGE_NT_HEADERS64 imageNTHeaders, DWORD fileBuffer) {
    IMAGE_DATA_DIRECTORY exportTable = imageNTHeaders->OptionalHeader.DataDirectory[0]; // Export table
    DWORD exportRVA = exportTable.VirtualAddress;
    DWORD exportSize = exportTable.Size;
    ...
    PIMAGE_EXPORT_DIRECTORY exportDir = (PIMAGE_EXPORT_DIRECTORY)(fileBuffer + exportRVA);
    if (exportDir == NULL) {
    ...
    printf("\t\t0x%x\t\t\tMajor Version\n", exportDir->MajorVersion);
    printf("\t\t0x%x\t\t\tMinor Version\n", exportDir->MinorVersion);
    printf("\t\t0x%x\t\t\tName RVA\n", exportDir->Name);
    printf("\t\t0x%x\t\t\tBase\n", exportDir->Base);
    printf("\t\t0x%x\t\t\tNumber of Functions\n", exportDir->NumberOfFunctions);
    printf("\t\t0x%x\t\t\tNumber of Names\n", exportDir->NumberOfNames);
    printf("\t\t0x%x\t\t\tAddress of Functions\n", exportDir->AddressOfFunctions);
    printf("\t\t0x%x\t\t\tAddress of Names\n", exportDir->AddressOfNames);
    printf("\t\t0x%x\t\t\tAddress of Ordinals\n", exportDir->AddressOfNameOrdinals);

    // Extract the Name of the DLL/EXE (which is a string located at the RVA)
    const char* exportName = (const char*)(fileBuffer + exportDir->Name);
    printf("\t\t%s\t\t\tModule Name\n", exportName);

    // Extract the function names, ordinals, and addresses
    DWORD* functionRVA = (DWORD*)(fileBuffer + exportDir->AddressOfFunctions);
    DWORD* nameRVA = (DWORD*)(fileBuffer + exportDir->AddressOfNames);
    WORD* ordinals = (WORD*)(fileBuffer + exportDir->AddressOfNameOrdinals);

    printf("\nExported Functions:\n");
    for (DWORD i = 0; i < exportDir->NumberOfNames; i++) {
        const char* functionName = (const char*)(fileBuffer + nameRVA[i]);
        DWORD functionAddress = functionRVA[ordinals[i]];

        printf("\tFunction %d: %s\n", i, functionName);
        printf("\t\tRVA: 0x%x\t\tAddress: 0x%x\n", functionAddress, fileBuffer + functionAddress);
    }
}
